...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            morphoresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            morphofile = await event.client.download_media(morphoresponse, "./temp")
            return await unzip(morphofile)
